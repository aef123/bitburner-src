...
import { recordPartialServerSnapshot } from "../serverSnapshots";
...
function executeScanAnalyzeCommand(depth: number, all: boolean): void {
  ...
  const printOutput = (node: Node, prefix = ["  "], last = true) => {
    ...
    if (!server) return;
    // UI-only: snapshot ONLY what this scan prints (skill/ports/RAM — never money/security), for
    // the terminal's target panel. Non-Server entities (hacknet/darknet) have no stats to snapshot.
    if (server instanceof Server) recordPartialServerSnapshot(server);
